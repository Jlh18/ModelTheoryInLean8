...
@[simp] def zero {n} : bdt n := bd_func RingConsts.zero

@[reducible] instance RingBDTHasZero {n} : has_zero (bdt n) := ⟨ zero ⟩
@[reducible] instance RingBDTHasZero' {n} : has_zero (bdt' n) := ⟨ zero ⟩

@[simp] def one {n} : bdt n := bd_func RingConsts.one
...
@[reducible] instance RingBDTHasAdd {n} : has_add (bdt n) :=
  ⟨ λ x, bd_app (bd_app (@bd_func RingSignature _ 2 RingBinaries.add) x) ⟩
@[reducible] instance RingBDTHasAdd' {n} : has_add (bdt'  n) :=
⟨ λ x, bd_app (bd_app (@bd_func RingSignature _ 2 RingBinaries.add) x) ⟩
...
namespace structureTo
  ...
  lemma apps_zero {n} : Π {t_ : dvector (bounded_term RingSignature n) 0},
  ...
  lemma apps_one {n} : Π {t_ : dvector (bounded_term RingSignature n) 0},
    bd_apps (@bd_func RingSignature _ 0 RingConsts.one) t_ = 1
  | [] := rfl

  lemma app_neg {n} {t : bounded_term RingSignature n} :
  ...
  lemma app_add {n} {s t : bounded_term RingSignature n} :
  ...
  lemma app_mul {n} {s t : bounded_term RingSignature n} :
...
namespace comm_ring_to
  ...
  lemma RealizeRingTheory :
    (structureTo.Structure A) ⊨ RingTheory :=
  begin
    intros ϕ h,
    repeat {cases h},
    {
    ...
    },
    {
      ...
      simp only [structureTo.FuncMap, fin.val_zero', structureTo.BinariesMap,
        structureTo.UnariesMap, dvector.last, eq_self_iff_true, realize_bounded_term_bd_app,
    ...
    },
    {
      ...
      simp only [mul, structureTo.realize_one, structureTo.FuncMap, fin.val_zero',
        structureTo.ConstMap, mul_one, structureTo.BinariesMap, dvector.last, eq_self_iff_true, one,
...
namespace mv_polynomial
  ...
  @[simp] lemma TermZero {n} : @Term n (bd_func RingConsts.zero) = 0 := rfl
  @[simp] lemma TermOne {n} : @Term n (bd_func RingConsts.one) = 1 := rfl
  ...
  begin
    ...
    simp only [structureTo.FuncMap, structureTo.BinariesMap,
      ...
      realize_bounded_term, mv_polynomial.eval_map, dvector.nth],
    simp only [ring_hom.to_fun_eq_coe, mv_polynomial.eval_map] at h,
    rw ← (h s (psum.inl rfl)),
...
namespace polynomial
  ...
  noncomputable def TermEvaluatedAtCoeffs {n} (as : dvector A n)
  ...
  @[simp] lemma TermEvaluatedAtCoeffsCoeff
  ...
  lemma TermEvaluatedAtCoeffsZero {n} {as : dvector A n} :
    TermEvaluatedAtCoeffs as (bd_func RingConsts.zero) = 0 :=
  begin
    simp only [TermEvaluatedAtCoeffs],
    unfold_coes,
  ...
  lemma TermEvaluatedAtCoeffsOne {n} {as : dvector A n} :
    TermEvaluatedAtCoeffs as (bd_func RingConsts.one) = 1 :=
  begin
    simp only [TermEvaluatedAtCoeffs],
    unfold_coes,
  ...
  begin
    rw pow_zero,
    simp only [one, TermEvaluatedAtCoeffsOne],
    refl,
  end
  | (n + 1) as :=
  begin
  ...
  (λ {t : bounded_term RingSignature (n + 1)}, Π {as : dvector A (n + 1)},
    ...
    (begin -- 0
      intro as,
      simp only [zero, lift_succ],
      repeat {rw TermEvaluatedAtCoeffsZero},
    end)
    (begin -- 1
      intro as,
      simp only [one, lift_succ],
...
namespace ModelTo
  ...
  @[simp] lemma realize_zero {n} {vec : dvector M.carrier n} :
    realize_bounded_term vec (@bd_func RingSignature _ 0 RingConsts.zero) dvector.nil = 0 := rfl

  @[simp] lemma realize_one {n} {vec : dvector M.carrier n} :
    realize_bounded_term vec (@bd_func RingSignature _ 0 RingConsts.one) dvector.nil = 1 := rfl
  ...
  begin
    have hAssoc := h.elim AddAssocInRingTheory,
