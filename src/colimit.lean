...
lemma canonical_map_inj_of_transition_maps_inj
  {D : directed_type} {F : directed_diagram D} (i : D.carrier)
  (H : ∀ {i} {j}, ∀ h : D.rel i j, function.injective (F.mor h))
...
def universal_map {D} {F : directed_diagram D} {V : cocone F} :
  colimit F → (V.vertex) :=
begin
  apply quotient.lift (λp : coproduct_of_directed_diagram F, V.map p.fst p.snd),
