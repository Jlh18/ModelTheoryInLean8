...
lemma is_consistent_henkin_theory_step {L} {T : Theory L} (hT : is_consistent T) :
...
begin
    ...
    (λf, (∃' f).cast1 ⟹ f) _ wit _ _),
  {
    ...
    simp only [wit_property, wit'],
    simp only [on_bounded_formula],
    simp only [subst0_bounded_formula_imp],
    simp only [fst_imp],
    simp only [and_true, eq_self_iff_true],
    rw [on_bounded_formula_ex, on_bounded_formula_cast1, subst0_bounded_formula],
    simp only [fol.bounded_preformula.cast_eq_fst,
      subst_bounded_formula_fst, fst_ex, fol.bounded_preformula.cast1_fst],
    rw subst_sentence_irrel,
  },
...
-- def henkin_language_chain_maps' (L : Language): Π x y, x ≤ y →
--   (@henkin_language_chain_objects L x →ᴸ @henkin_language_chain_objects L y)
...
def henkin_language_chain {L : Language} :
  directed_diagram_language directed_type_of_nat :=
...
lemma henkin_theory_chain_inclusion_step {L} {T : Theory L} {i : ℕ} {f ∈ henkin_theory_chain T i} :
